...
gulp.task('freeze', ['html-dev'], function () {
    var fs = require('fs');
    var rData;
    fs.readFileSync('public/app.html', {encoding: 'utf-8'}, function (err, data) {
        if (err) {
            throw err;
        }

        var sData = data.toString();
        rData = sData.replace('<%= hashAppcss %>', md5File('public/app.css').substring(0, 10))
        ...
        fs.writeFileSync('./public/app.html', rData, function (err) {
            if (err) {
                throw err;
            }
        });
